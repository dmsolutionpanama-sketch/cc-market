...
import { X, Send, PhoneCall, ShieldCheck, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden my-auto">
        ...
        {/* Call Platform Notice Banner */}
        <div className="bg-amber-50 border-b border-amber-200 p-4 flex items-start gap-3 text-xs text-amber-900">
          <ShieldCheck className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
          <div>
            <span className="font-bold block mb-0.5">{t.callPlatformNotice}</span>
            <p className="text-amber-800">
              Coordinamos la negociación, contrato y cronograma de publicaciones a través de nuestro centro de atención telefónica especializado.
            </p>
          </div>
        </div>
        ...
        <div className="p-6 text-slate-800">
          {submitted ? (
            <div className="p-8 text-center bg-emerald-50 border border-emerald-200 rounded-xl my-4">
              <CheckCircle2 className="w-12 h-12 text-emerald-600 mx-auto mb-3" />
              <h3 className="text-lg font-bold text-emerald-950 mb-1">{t.successMessage}</h3>
              <p className="text-xs text-emerald-800">
                Nos comunicaremos con tu empresa mediante nuestra plataforma integrada de llamadas en breve.
              </p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4 text-xs font-medium">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                  />
                </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold rounded-xl text-xs flex items-center gap-2 shadow-md transition-all cursor-pointer"
                >
